...
public class NumMoneyConvertHelper {
	...
	//转成中文的大写金额
	public static String toChinese(String str,Integer whole) {
		//判断输入的金额字符串是否符合要求
		if (StringHelper.isEmpty(str) || !str.matches("(-)?[\\d]*(.)?[\\d]*")) {
			return "抱歉，请输入数字！";
		}
		...
		int[] integers = null;
		try{
			 integers = toIntArray(integerStr);//整数部分数字
		}catch (Exception e){
			return "抱歉，请输入数字";
		}
		//判断整数部分是否存在输入012的情况
		...
		if(whole == 1){
			result = isWhole ? result+"整" : result;
		}
